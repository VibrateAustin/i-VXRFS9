...
namespace i_VXRFS.NewAndOpenQuantitationAnalysisApplication
{
    public partial class CheckAngle : Form
    {
        ...
        public string identifyStr="new";
        public List<double> RawData = new List<double>();
        private newquantitationanalysisapplication phdAForm;
        public CheckAngle(newquantitationanalysisapplication f1)
            :this()
        {
            this.phdAForm = f1;
        }
        private quantitationanalysisapplication phdAForm2;
        public CheckAngle(quantitationanalysisapplication f2)
            : this()
        {
            this.phdAForm2 = f2;
        }
        //public List<string> sqlSelectA(string str) //读取数据表信息，返回列表
        //{
        //    List<string> AngelData = new List<string>();
        ...
        {
            //string Astr = "select * from tb_checkAngel where element='" + getelementparameter[0] + "';";
            ...
            SearchDataFromTxt(SysPath, ref dataA);
                ...
                else
                {
                    ...
                    txtb_scantime.Text = "30";
                    txtb_step.Text = "0.04";//step的值估计需要变动
                    txtb_bgTime.Text = "10";
                }
            cbox_mask.Text = "20";
            ...
            dgv_bgPoint.Rows[index].Cells[0].Value = getelementparameter[9].ToString();
            dgv_bgPoint.Rows[index].Cells[1].Value = getelementparameter[10].ToString();
            dgv_bgPoint.Rows[index].Cells[2].Value = getelementparameter[11].ToString();
            dgv_bgPoint.Rows[index].Cells[3].Value = getelementparameter[12].ToString();
        ...
        private void btn_ok_Click(object sender, EventArgs e)
        {
            //在上一级dgv中显示
            //8-12.21
            getelementparameter[8] = Math.Round(Convert.ToDouble(txtb_peakposition.Text),4).ToString();
            if (identifyStr == "new")
            {
                phdAForm.dgv3.Rows[DgvIndex].Cells[8].Value = getelementparameter[8];
                for (int i = 0; i < 4; i++)
                {
                    if (Convert.ToDouble(dgv_bgPoint.Rows[0].Cells[i].Value) != 0)
                        phdAForm.dgv3.Rows[DgvIndex].Cells[9+i].Value = dgv_bgPoint.Rows[0].Cells[i].Value;
                }
                phdAForm.dgv3.Rows[DgvIndex].Cells[21].Value = DateTime.Now.ToString("yyyy/MM/dd");
            }
            else if (identifyStr == "open")
            {
                phdAForm2.dgv3.Rows[DgvIndex].Cells[8].Value = getelementparameter[8];
                for (int i = 0; i < 4; i++)
                {
                    if (Convert.ToDouble(dgv_bgPoint.Rows[0].Cells[i].Value) != 0)
                        phdAForm2.dgv3.Rows[DgvIndex].Cells[9 + i].Value = dgv_bgPoint.Rows[0].Cells[i].Value;
                }
                phdAForm2.dgv3.Rows[DgvIndex].Cells[21].Value = DateTime.Now.ToString("yyyy/MM/dd");
            }
           ...
           // for (int m = 0; m < 4; m++)
           // {
           //     if ( m < axisData.Count && Convert.ToDouble(dgv_bgPoint.Rows[0].Cells[m].Value) != 0)
           //     {
           //         //dgv_bgPoint.Rows[0].Cells[m].Value = "0"; 
           //         str += ",bg" + (m + 1) + "=" + dgv_bgPoint.Rows[0].Cells[m].Value;
           //     }
           //     else
           //     {
           //         str += ",bg" + (m + 1) + "=' '";
           //     }
           // }
           // getelementparameter[21] = DateTime.Now.ToString("yyyy-MM-dd");
           // //getelementparameter[22] = txtb_scantime.Text.ToString();
           ////更新数据到数据库中
           // str = "update tb_quantitationelementchannel set angle='" + getelementparameter[8] + "'"
           //     +str+ ",dateTime='" + getelementparameter[21] + "' where name='" + getelementparameter[0] + "';";
           // sqlData(str);
           // //保存到另外一个数据表中
           // bool i = false;
           // string strF = "select * from tb_checkAngel where element='" + getelementparameter[0] + "';";
           // i = sqlSelect(strF);
           // if (i==true)
           // {
           //     //更新数据
           //     string strU = "update tb_checkAngel set peak='" + getelementparameter[8] + "',startA=" +
           //     txtb_startangle.Text + ",endA=" + txtb_endangle.Text + ",time=" + txtb_scantime.Text + ",bgTime=" + txtb_bgTime.Text
           //     + ",step=" + txtb_step.Text + " where element='" + getelementparameter[0] + "';";
           //     sqlData(strU);
           // }
           // else
           // {
           //     //插入数据
           //     string strI = "Insert into tb_checkAngel(element,peak,startA,endA,time,bgTime,step) values('"+getelementparameter[0]
           //         +"','"+getelementparameter[8]+"',"+txtb_startangle.Text +","+txtb_endangle.Text
           //         + "," + txtb_scantime.Text + "," + txtb_bgTime.Text + "," + txtb_step.Text + ");";
           //     sqlData(strI);
           // }
