...
{
    public partial class newquantitationanalysisapplication : Form
    {
        //声明两个变量，获得定量分析程序创建名和创建时间
        public string newApplicationname=string.Empty;
        public string newApplicationtime=string.Empty;
        public int isApplicationnameexist;
        public string newApplicationTempname ="tempFile";//放临时文件的文件夹
        public newquantitationanalysisapplication()
        {
            InitializeComponent();
        ...
        private void newquantitationanalysisapplication_Load(object sender, EventArgs e)
        {
            try
            {
                //如果是复制过来，需要加载所有的数据过来。同“打开定量分析程序”窗口
                if (isApplicationnameexist == 1) //程序名是复制过来的
                {
                    //加载信息过来
                    //读取在该定量分析程序名文件夹文件中的数据，并显示在窗口相应的位置
                    string filepath = XRF_function.QuantitationApplicationPath + "\\" + newApplicationname + "\\" + newApplicationname + ".txt";
                    ...
                    lab_applicationname.Text = this.newApplicationname;
                    ...
                    cbox_saveFilepath.Text = this.newApplicationname;
                    cbox_revise.Text = newApplicationname;
                    cbox_reviseupdate.Text = newApplicationname;
                    cbox_getinputdata.Text = newApplicationname;
                    ...
                    while ((index[3] + k + 1) < index[4])
                    {
                      ...
                      //  InsertDataToDgv.TxtDataInputToDgv(XRF_function.LinkDataBasePath, "tb_additivecompound", getaddtivestr);
                    ...
                    ReadData.Clear();
                  //  XRF_function.showdatagridview(dgv_additivecompound, "tb_additivecompound");
                    ...
                    while ((index[4] + k + 2) < index[5])
                    {
                       ...
                       // InsertDataToDgv.TxtDataInputToDgv(XRF_function.LinkDataBasePath, "tb_quantitationcompound", getelementComstr);
                        k++;
                    }
                    if (ReadData.Count > 0)
                        SysConfig_FileSave.DListAddToDgv(dgv_quantitationcompound, ReadData);
                    ReadData.Clear();
                    //XRF_function.showdatagridview(dgv_quantitationcompound, "tb_quantitationcompound");
                    ...
                    while ((index[5] + k + 1) < index[6])
                    {
                        ...
                        //InsertDataToDgv.TxtDataInputToDgv(XRF_function.LinkDataBasePath, "tb_quantitationelementchannel", getelementParastr);
                    ...
                    ReadData.Clear();
                    //XRF_function.showdatagridview(dgv_elementchannel, "tb_quantitationelementchannel");
                    ...
                    while ((index[6] + k + 2) < (allinfo.Count - 1) && allinfo[(index[6] + k + 2)] != " ")
                    {
                        ...
                        //InsertDataToDgv.TxtDataInputToDgv(XRF_function.LinkDataBasePath, "tb_deductbg_quantitation", getbgparastr1);
                    ...
                    ReadData.Clear();
                   // XRF_function.showdatagridview(dgv_deductbg_quantitation, "tb_deductbg_quantitation");
                ...
                {
                    //对分析程序进行参数设置后的数据都保存在该定量分析程序名文件夹中
                    lab_applicationname.Text = this.newApplicationname;
                    txt_createsoftname.Text = "i-VXRFS";
                    txt_updatesoftname.Text = "i-VXRFS";
                    txt_createtime.Text = this.newApplicationtime;
                    txt_updatetime.Text = this.newApplicationtime;
                    txt_createsoftname.ReadOnly = true;
                    txt_updatesoftname.ReadOnly = true;
                    this.txt_createtime.ReadOnly = true;
                    this.txt_updatetime.ReadOnly = true;
                    ...
                    cbox_getinputdata.Text = newApplicationname;
                    //设置默认值
                    numUD_xrfvacuumlocktime.Text = "1";
                    cbox_medium.Text = "真空";
                    numUD_delaytime.Text = "1";
                    cbox_mask.Text = "10";
                    cbox_priority.Text = null;
                    cbox_cup.Text = "10";
                    cbox_samplestate.Text = "熔片";
                    cbox_samplesize.Text = "固定";
                    cbox_sampleLOI.Text = "无";
                    txt_samplethick.Text = null;
                    txt_samplediameter.Text = null;
                    txt_additive.Text = null;
                    txt_totalmass.Text = null;
                    cbox_normalize.Text = "100";
                    txt_measuremintime.Text = null;
                    txt_measuremaxtime.Text = null;
                    //cbox_bgmethod.Text = "无";
                    //txt_bgchannelelement.Text = null;
                    txtb_samplepreparedescribe.Text = null;
        ...
        {
            //先把dgv数据表中的数据复制到临时文件*.txt中，再清空dgv数据
            ...
            //string[] dgv_elementchanneldata = { };
            //dgv_elementchanneldata = XRF_function.GetDGVData2(dgv_elementchannel);
            ...
            if (isApplicationnameexist == 1) //程序名是复制过来的，需要把保存在临时文件中的数据 移动到修改后的文件中？？？
            {
                //弹出窗口提示修改程序名
                changeApplicationName changeapplicationname = new changeApplicationName();
                changeapplicationname.changename = this.newApplicationname;//把定量分析程序名传递给子窗口变量
                changeapplicationname.Show();
                isApplicationnameexist = 0;
                this.Close();
            }
            else
            {
                ...
                string newpath = null;
                string oldpath = XRF_function.QuantitationApplicationTempPath + "\\" + newApplicationname + "tempfile.txt";//临时文件存放地址
                newpath = XRF_function.QuantitationApplicationPath + "\\" + newApplicationname + "\\" + newApplicationname + ".txt";
                FileInfo finfo = new FileInfo(oldpath);
                finfo.MoveTo(newpath); //移动到的目标文件夹中，并指定新的文件名
                string SaveResultsPath = XRF_function.QuantitationAnalysisResultsPath + "\\" + newApplicationname; //在定量结果路径中创建存放结果的文件夹
           ...
           /* if (!File.Exists(temppath) == true)
            {
                ...
                finfo.Create(); //创建保存数据的文本文件 
            } */
           // FileStream saveTempfile = new FileStream(temppath, FileMode.OpenOrCreate, FileAccess.ReadWrite);
            ...
            try
            {
                // Thread.Sleep(1000);
                //开始写入各种数据到临时文件中
              //  saveSW.WriteLine(newApplicationname);
        ...
        private void newquantitationanalysisapplication_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void btn_checkangle_Click(object sender, EventArgs e)
        {
            if (dgv_elementchannel.SelectedRows.Count > 0)
            {
                ...
                checkAngleform.identifyStr = "new";
                checkAngleform.Show();
            }
            else
            {
