...
{
    public partial class CheckEDXRF : Form
    {
        public CheckEDXRF()
        {
            InitializeComponent();
        ...
        public CheckEDXRF(PerformanceTest_mainPage f1)
            :this()
        {
            this.phdAForm = f1;
        }
        ...
        private void CheckEDXRF_QuantitationAnalysis_Load(object sender, EventArgs e)
        {
           ...
           // SearchDataFromTxt(SysPath, ref dataA);
            ...
            txtb_phaL.Text = getelementparameter[13];
            txtb_phaU.Text = getelementparameter[14];
            ...
            float factor = (float)(Convert.ToDouble(txtb_peakposition.Text) + 2 - Convert.ToDouble(txtb_peakposition.Text)) / 9;
            for (int i = 0; i < 10; i++)
            {
                txt = (Math.Round(Convert.ToDouble(txtb_peakposition.Text)-1 + i * factor, 4)).ToString();
